// server4.js - NodeJS server for the PiThermServer project.
...
function insertTemp(data){
  ...
  // the five ‘?’s are placeholders for date/time, attic temp, outside temp, outside dewpoint, and outside humidity
...
function readTemp(callback){
  ...
  var ds18b20Buf = fs.readFileSync('/sys/bus/w1/devices/28-000002afefb2/w1_slave');

  // Read data from file (using fast node ASCII encoding).
  var data = ds18b20Buf.toString('ascii').split(" "); // Split by space
  ...
  var acuriteBuf = fs.readFileSync('/home/pi/rtl_433/temp.json');

  // Read data from file (using fast node ASCII encoding).
  var data = acuriteBuf.toString('ascii').split(", "); // Split by comma+space
  ...
  // Extract humidity from string
  var humAr  = parseFloat(data[data.length-1].split(" : ")[1]);
  console.log('humAr = ' + humAr);
  ...
  // round to one decimal place
  var dewPointF = Math.round(dewPointF * 10) / 10;
